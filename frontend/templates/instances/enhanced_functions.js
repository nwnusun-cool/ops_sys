...
function toggleView() {
    ...
    if (currentView === 'card') {
        ...
        viewToggle.innerHTML = '<i class="fas fa-th-large me-1"></i>表格视图';
        currentView = 'table';
...
function renderInstanceTable(instances) {
    ...
    if (instances.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted">没有找到实例</td></tr>';
        return;
    }
        ...
        <tr data-instance-id="${instance.id}" ${selectedInstances.has(instance.id) ? 'class="table-active"' : ''}>
            ...
            <td>
                ...
                ${instance.metadata && instance.metadata.destroy_at ? 
                    `<br><small class="text-danger"><i class="fas fa-clock"></i> 销毁时间: ${formatTime(instance.metadata.destroy_at)}</small>` : ''
                }
            </td>
            <td>
...
function setDestroyTimer(instanceId, instanceName) {
    ...
    document.getElementById('destroyDateTime').value = '';
